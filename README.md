...
- **Files and app.** FIT, TCX and GPX with the same points and laps. Elevation, pace, heart rate (demand and recorded) and cadence on one shared playhead. Laps and splits, calories, a share link, English and Russian, metric and imperial units.
...
Runsketch делает файл тренировки из маршрута, нарисованного на карте. Вы задаёте маршрут, спортсмена и цель: темп, скорость, время на финише или уровень нагрузки. Дальше тренировка моделируется посекундно, и в FIT, TCX или GPX записывается то, что записали бы часы: координаты, высота, пульс, каденс, температура, а на велосипеде ещё и мощность.

Это статический сайт на GitHub Pages под лицензией PolyForm Noncommercial 1.0.0: пользоваться кодом и менять его можно бесплатно в некоммерческих целях со ссылкой на автора, зарабатывать на нём нельзя (подробнее в разделе [License](#license)). Редактор карты, расчёт и запись файлов работают прямо в браузере. Аккаунта, ключей API и своего сервера нет.

### Как это работает

Участки между точками, которые вы ставите на карте, прокладываются по дорогам, тропам или горным маршрутам через открытые сервисы маршрутизации. Высоту берём из открытой модели рельефа через каждые 5 м, убираем выбросы вроде мостов и зданий, сглаживаем и только потом считаем уклон.

Потом движок проходит маршрут с шагом в секунду. Сначала он решает, как человек движется: в гору медленнее, на пологом спуске чуть быстрее, на крутом тормозит, а там, где бегун перешёл бы на шаг, идёт пешком. По скорости, уклону и покрытию считается, сколько кислорода нужно мышцам, и пульс догоняет эту потребность с задержкой. Каденс, мощность, точки GPS, высота и температура часов получаются в том же проходе. Графики на экране и все три файла строятся из одного результата, а те же настройки с тем же сидом дают тот же файл.

Формулы, константы и источники описаны в [`docs/physiology.md`](docs/physiology.md) (по-английски), а отдельные части — на своих страницах: [рельеф и покрытие](docs/terrain.md), [усталость](docs/fatigue.md), [погода](docs/weather.md), [высокогорье](docs/mountaineering.md) и [что записывают часы](docs/recording.md). На каждой странице перечислены числа, которые пока остаются нашей оценкой, а не измерением.

### Возможности

- **Маршрут.** Точки ставятся щелчком и перетаскиваются. Есть отмена и повтор, замыкание круга, «туда и обратно», разворот, импорт GPX и TCX, поиск мест. Профили прокладки: дороги и дорожки, тропы, высокогорье, велосипед, шоссейный и горный велосипед, по прямой. В меню примеров есть холмистый круг по Монжуику в Барселоне, восхождения на Эльбрус, Казбек, Монблан, Килиманджаро и Аконкагуа и два трека в Непале.
- **Активность и спортсмен.** Бег, велосипед, ходьба, поход и альпинизм. В профиле спортсмена: возраст, пол, вес, рост, пульс покоя и максимальный, уровень подготовки и датчик пульса (нагрудный или на запястье).
- **Цель.** Средний темп, скорость или время на финише либо уровень нагрузки («Легко», «Ровно», «Темпово», «Гонка»), который подбирается под этого спортсмена на этом маршруте. Можно указать и средний пульс: темп останется прежним, а движок подберёт такой уровень подготовки, при котором получится этот средний. Ровная, негативная или позитивная раскладка, разброс темпа, остановки, шум GPS, сид.
- **Темп.** Уклон, покрытие и сложность пути по тегам OpenStreetMap, высота над уровнем моря, разминка, повороты, остановки и усталость, которая копится от нагрузки, подъёмов и спусков, а не просто с течением времени. На крутых участках бегун переходит на шаг и обратно. На велосипеде скорость получается из баланса мощности (Martin et al., 1998).
- **Пульс.** Целевое значение считается от потребности в кислороде через резерв пульса. Записанный пульс идёт за ним двумя составляющими, быстрой и медленной: обе снижаются медленнее, чем растут, а у тренированных людей реагируют быстрее. Выше порога добавляется медленный компонент, со временем набегает кардиодрейф, в жару он сильнее. Поверх всего идёт шум нагрудного или наручного датчика.
- **Погода и высота.** Погода на выбранные дату и место подставляется сама и меняется по ходу тренировки: температура, ветер, дождь или снег, мокрое покрытие. Аэробные возможности снижаются с высотой, и это учитывается вплоть до высоких гор.
- **Записанные сигналы.** Ошибка GPS медленно плывёт, а не скачет от точки к точке. Барометрическая высота со своим дрейфом, температура часов, нагретых рукой, беговая динамика и автопауза на остановках в FIT, изредка пропущенные секунды.
- **Файлы и интерфейс.** FIT, TCX и GPX с одними и теми же точками и кругами. Графики высоты, темпа, пульса (потребность и записанный) и каденса с общим ползунком. Круги и сплиты, калории, ссылка, которой можно поделиться, английский и русский, метрические и имперские единицы.

### Чем отличается от других генераторов

Большинство известных нам генераторов тренировок платные. fakemy.run продаёт скачивания пакетами токенов, в сентябре 2026 года выходило примерно 0,42–0,60 доллара за файл. SimuRun, Dibma и Fake My Stats берут деньги за файл, кредитами или по подписке. Нарисовать маршрут и посмотреть превью там обычно можно бесплатно, а платить нужно за сам файл.

Главное техническое отличие в том, откуда берётся пульс. fakemy.run считает его как `avg · (0.7 + 0.3 · sin(π · i/N))` плюс независимый случайный шум в каждой точке трека, где `i/N` означает положение точки в списке. Поэтому пульс ниже всего на старте и финише и выше всего ровно на середине, какой бы ни был рельеф. Подъём, ускорение или остановка на него не влияют, а раз шум в каждой точке новый, инерции тоже нет. Темп там получает независимый шум на каждом коротком отрезке и от уклона не зависит. Файл там только GPX, без каденса, температуры и мощности. Точки лежат на линии маршрута с независимым дрожанием меньше метра, а графики превью строятся из своих случайных чисел и со скачанным файлом не совпадают.

У нас рельеф задаёт скорость, скорость задаёт потребность в кислороде, а пульс идёт за потребностью с запаздыванием. На графике в английской части выше записанный пульс первую половину подъёма идёт примерно на 6 ударов ниже потребности, к вершине обгоняет её, а сразу после вершины держится на 13–15 ударов выше неё около полуминуты и сходится с ней примерно через две минуты. Ошибка GPS связана во времени и по расстоянию, а превью и файлы строятся из одного и того же расчёта.

Два платных сервиса выкладывают код генерации прямо на странице, с комментариями, и его можно прочитать. Fake My Stats считает темп по тому же полиному Минетти (2002), что и мы: статья названа в комментарии. Пульс у них нарастает с постоянной времени 28 секунд и восстанавливается с 55. У нас быстрый вагусный ответ отделён от медленного симпатического, поэтому отношение зависит от перехода: 2,0 между усилиями и 3,2 после полной остановки. Что из этого ближе к истине — вопрос открытый: работы, где засекали оба направления, делались на велоэргометре и на клинических группах, а измерений на бегунах по пересечённой местности мы не нашли. На сайте написано, что пульс «нарастает и восстанавливается с задержкой», и код действительно это делает. У Dibma поверх уклона лежат накопительная усталость от набора высоты и отдельный член дрейфа, а ошибка GPS — процесс AR(1): она переходит из точки в точку, а не рождается заново каждую секунду, и это правильная форма для приёмника. Файлы — FIT с каденсом, остановками и кругами.

Различия после этого куда уже, и каждое можно проверить по полевым данным. Скорость спуска Fake My Stats берёт прямо из кривой стоимости бега, как если бы усилие держалось постоянным: выходит 1,67 равнинной скорости на −10 % и 2,0 на −20 %, и код обрезает это до 1,28–1,40. Обрезка держит числа в разумных пределах, но кривая остаётся плоской и дальше уже не опускается. Рекорды горных забегов ведут себя иначе: около 1,23 на −10 %, 1,03 на −20 %, а к −30 % бегун идёт медленнее, чем по равнине, потому что на крутом спуске приходится тормозить. В их пульс уклон входит только в плюс, так что спуск на него не влияет, — у нас быстрый спуск пульс не отпускает. У Dibma множитель подъёма упирается в 1,50 начиная с +12 %, тогда как подъём в +20 % стоит ближе к 2,1; дрейф и поздняя усталость привязаны у них к доле пройденного, а не ко времени, поэтому получасовая пробежка и пятичасовая дрейфуют одинаково, а запаздывание пульса применяется к каждой записи, и постоянная времени зависит от частоты записи. У SimuRun пульс читает рельеф с запаздыванием, а темп уклона не видит вовсе — расстояние, делённое на одну среднюю скорость, — и два канала описывают разные пробежки.

На подъёмах мы из этих трёх оказываемся самыми быстрыми, а не самыми медленными. На +10 % кривая равного усилия по Минетти даёт 0,60 от равнинной скорости, рекорды — 0,65, а наш движок около 0,74. За этим стоит наблюдение, что на холмах бегун не удерживает усилие постоянным (Townshend 2010); это выбор модели, а не решённый вопрос.

Десять таких различий не обсуждаются, а измерены — в [`docs/comparison.md`](docs/comparison.md): подъёмы и спуски против рекордов горных забегов, запаздывание и дрейф пульса, падение темпа на марафоне, посекундная фактура против реальной записи с часов, остановки и жара. Там же сказано, где мы проигрываем: подъёмы у нас быстрее рекордов, и это осознанный выбор; наш бегун по умолчанию сдаёт к финишу меньше обычного марафонца; холод на темп не влияет вовсе.

Из бесплатного есть gpx.studio, открытый редактор маршрутов на похожих открытых сервисах. По умолчанию он раскладывает заданное общее время по маршруту с учётом уклона, отдавая подъёмам больше времени; постоянная скорость — это ручной вариант. Пульс, каденс и мощность он не придумывает, но сохраняет их, если они уже есть в импортированном файле. Открытые генераторы на GitHub, которые мы смотрели, в основном требуют токен Mapbox, ключ API маршрутизации или собственный сервер, и большинство из них пульс вообще не пишет.
