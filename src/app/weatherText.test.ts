...
describe('weather texts', () => {
  it('summarise temperature, the rain window at the route clock and the wind', () => {
    expect(weatherSummaryText('metric', summary, START, 120)).toBe('3…11 °C · rain 08:00–09:15 · wind SW → NW 7–25 km/h');
    expect(weatherSummaryText('imperial', { ...summary, windFromEnd: 225 }, START, 120)).toBe('37…53 °F · rain 08:00–09:15 · wind SW 4–16 mph');
  });
  ...
  it('say dry, calm and snow, and write a true minus', () => {
    ...
    expect(weatherSummaryText('metric', cold, START, 0)).toBe('−4 °C · dry · calm');
    expect(weatherSummaryText('metric', { ...summary, snowShare: 0.8 }, START, 0)).toMatch(/· snow 06:00–07:15 ·/);
  });

  it('name sources, compass points and clock times', () => {
    expect(weatherSourceText('climatology', 2019)).toBe('Typical for this date (weather of 2019)');
